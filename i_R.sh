...
while true; do
    ...
    case $yn in
	    ...
	    cd "$(ls -dt R-*/ | head -1 )";
	    ./configure --enable-memory-profiling --enable-R-shlib --with-blas --with-lapack --with-system-zlib --with-system-bzlib --with-system-xz --with-tcltk  --with-cairo --with-libpng --with-jpeglib --with-libtiff --with-x=no;
	    make;
	    sudo make install;
	    break;;
